...
class Checkout extends Component {
    ...
    async handleCaptureCheckout(e) {
        ...
        if (!loading && user) {
            ...
            let auth0User = await this.context.getUser(user.sub);
            commerceCustomer = await this.context.getCommerceCustomer(auth0User.data.app_metadata.commerceID);

            if ((commerceCustomer == null)) {
                commerceCustomer = await this.context.createCommerceCustomer(user.email);
                await this.context.updateAuthCustomerCommerceID({ id: user.sub, commerceID: commerceCustomer.data.id })
            }
        }
        try {
            const orderData = {
